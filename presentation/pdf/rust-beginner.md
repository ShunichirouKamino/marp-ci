...
- ツールチェーン管理
  - `rustup`により、これまで登場した`rustc`や`cargo`等のツール群を一式インストールしてくれます。また、`rust-analyzer`や`rls`といった LSP、`rust-fmt`といったフォーマッタ、`clippy`といった Linter ツールも管理されます。
...
![.](./img/rust-beginner/rust-rls1.gif)
